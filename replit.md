...
- **Hard dev→prod sync approval gate**: before any production sync, create a read-only divergence report comparing dev and prod across **all tables**, not only the tables the requested change is about. Enumerate every prod-only row, dev-only row, changed row, and schema difference that the sync could affect. A request scoped to one change (for example, “sync Treasure Garden PDF links”) is **not** approval for unrelated catalog changes or any deletion. Stop and show the exact affected records in plain English; do not run the mutating sync until the user explicitly approves each unexpected difference. In particular, any prod-only catalog deletion/replacement must be blocked unless explicitly approved.
