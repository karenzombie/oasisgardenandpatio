...
- **Hard dev→prod sync approval gate**: before any **data** sync, identify the exact tables and rows the proposed operation can mutate, overwrite, delete, or cascade-delete, and create a read-only divergence report for that affected scope. Do **not** run a blanket all-table comparison for a code-only publish, and do not treat the expected dev/prod separation in operational tables as a blocker or an approval request. Operational tables are never sync targets. A request scoped to one data change (for example, “sync Treasure Garden PDF links”) is **not** approval for unrelated catalog changes or any deletion. Stop and show the exact affected records in plain English; do not run the mutating sync until the user explicitly approves each unexpected difference. In particular, any prod-only catalog deletion/replacement must be blocked unless explicitly approved.
...
- **Post-launch DB sync policy (takes over at launch)**:
  - Prod becomes the source of truth for ALL non-product data: customers, orders, vendor orders, staff users, and anything else a customer or staff member can create or modify. Dev-to-prod syncs must NEVER overwrite, wipe, or alter this data in prod, ever, under any circumstance.
   - Every **data sync** (dev→prod) requires a scoped divergence analysis across the tables the operation can affect first, producing a report that categorizes every difference found. A code-only publish does not require comparing unrelated operational tables:
   ...
   - I never make my own data-sync decisions on any in-scope divergence. Every affected difference is flagged, described in plain English, and held for my explicit approval before any data action is taken — no silent overwrites, no assumptions, no "safe to skip." Differences in tables explicitly excluded from the operation are not surfaced as sync blockers.
...
- **Unexpected sync differences require approval**: report them before the sync, and stop all deletion/replacement work until the user explicitly approves those specific records. Never treat a successful sync or a broad catalog-sync request as approval for unrelated differences.
