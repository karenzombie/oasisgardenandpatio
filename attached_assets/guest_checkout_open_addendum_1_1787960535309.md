...
The gate is four conditions in two files, but flipping only those conditions
...
- `OrderConfirmation.tsx` has no auth gate, and the order fetch behind it,
  `GET /account/orders/:orderNumber`, has an explicit guest path: a guest may
  fetch an order whose number is in `req.session.guestOrders`, which `/checkout`
  populates. The guest confirmation page already works end to end.
- Guests already see the inline billing and shipping address forms. The
  authenticated-only conditions in those sections gate the saved-address picker
  only, and guests fall through to the inline inputs. **Do not restructure the
  address sections.**
...
**About the line numbers below.** They are accurate against the current main
branch as of this brief. They will shift as soon as you start deleting blocks,
so the numbers cited in the later items will no longer match once Steps 1 and 2
are done. Locate every item by the symbol or text named in it, and treat the
line number only as a starting point. Never edit by line number alone.

### File 1: `artifacts/web/src/pages/Cart.tsx`

1. **Line 37:** the `isDemoUser` constant, which is assigned the authenticated
   flag. Remove it entirely. It is used in exactly one place.
2. **Lines 328 to 357:** the `isDemoUser` conditional block. Keep the enabled
...
4. **Line 92, the `useGetCheckoutPaymentConfig` query:** it is currently enabled
   only for authenticated users. It must run for everyone. Without this a guest
   never receives the Accept.js credentials and the HostedForm can never render.
   Also update the comment above it, which currently states that guests see the
   under-construction UI and never trigger payment.
5. **Line 82, the `useListAccountAddresses` query:** leave its authenticated-only
   condition exactly as is. Guests have no address book and the backend rejects
   a guest who sends a saved address ID. This one must stay gated.
6. **Lines 886 to 908:** delete the entire guest-only "Payment" section
   containing the under-construction notice, including the three-line explanatory
   comment immediately above it at 886 to 888, which describes the split being
   removed and would otherwise be left behind describing code that no longer
   exists.
7. **Lines 992 to 1066:** the authenticated conditional wrapping the security
   note, the `HostedForm`, and the address and payment hints. Render that entire
   authenticated branch for everyone. Delete the else branch: the disabled
   "Place Order" button and its under-construction paragraph.
