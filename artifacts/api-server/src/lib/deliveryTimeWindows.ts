...
// Keep this list in sync with artifacts/web/src/staff/lib/deliveryTimeWindows.ts.
...
/**
 * Normalizes a stored time value (Postgres may return "HH:MM:SS" or
 * "HH:MM:SS.ffffff") to the plain "HH:MM:SS" form used as the map key.
 */
function normalizeTimeValue(value: string): string {
  const parts = value.split(":");
  if (parts.length === 2) return `${value}:00`;
  if (parts.length === 3) return `${parts[0]}:${parts[1]}:${parts[2].slice(0, 2)}`;
  return value;
}
