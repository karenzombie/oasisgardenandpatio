  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (signIn.status === "complete") {
        ...
        // Navigation is driven by the isSignedIn guard on the sign-in page —
        // do not navigate here. Firing it synchronously races ahead of
        // clerk-sync (POST /auth/clerk-sync), causing /account to see a 401
        // from /api/auth/me and bounce back to the sign-in page.
      } else {
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-2">
          <label htmlFor={emailId} className="text-sm font-medium text-foreground block">
            Email address
          </label>
          <input
            id={emailId}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label htmlFor={passwordId} className="text-sm font-medium text-foreground block">
              Password
            </label>
            <a
